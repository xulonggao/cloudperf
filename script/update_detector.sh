...
if [ "${deploy_type}" == "aws" ]; then
    ...
    for deploy_region in ${deploy_location}; do
        ...
        COMMAND_ID=$(aws ssm send-command \
            ...
            --parameters "commands=[\"curl -sSL https://raw.githubusercontent.com/tansoft/fping/refs/heads/develop/setup/update-linux.sh | sed 's/fping-job/${detector_type}/g' | bash\"]" \
    ...
    ssh ${deploy_location} "curl -sSL https://raw.githubusercontent.com/tansoft/fping/refs/heads/develop/setup/update-linux.sh | sed 's/fping-job/${detector_type}/g' | sudo bash"
else
